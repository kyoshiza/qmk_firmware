...
#define SS_ALNUM(x) SS_TAP(X_CAPS) x SS_TAP(X_CAPS)
...
//        case NG_P   : send_string(SS_ALNUM(SS_TAP(X_COMM))); break; // ，
...
// 親指左同時押し
        switch(nicola_m_key) {
            case NG_1   : send_string(SS_ALNUM(SS_LSFT(SS_TAP(X_SLSH)))); break;
            case NG_2   : send_string(SS_ALNUM(SS_TAP(X_SLSH))); break;
            case NG_3   : send_string(SS_ALNUM(SS_LSFT(SS_TAP(X_EQL)))); break; // ~
