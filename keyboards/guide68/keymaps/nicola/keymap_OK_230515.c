...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
     ...
     * ├────┬──┴─┬─┴──┬┴───┴┬──┴──┬┘  ┌┴───┴┬──┴─┬─┴──┬┴───┼───┼───┼───┤
     * │ L1     │  LGUI  │  LAlt  │   Spc    │   Spc    │    │   Spc    │  Spc   │  RAlt  │  RGUI  │  ←  │  ↓  │  →  │
     * └────┴────┴────┴─────┴─────┘    └─────┴────┴────┴────┴───┴───┴───┘
     */
  [_QWERTY] = LAYOUT( \
      ...
      MO(_FUNC),    KC_LGUI,   KC_LALT,        KC_EISU,  KC_SPC,  KC_SPC, KC_KANA2,         KC_RALT, KC_RGUI, KC_LEFT,KC_DOWN,KC_RGHT),
...
// NICOLA親指シフト
  ...
  [_NICOLA] = LAYOUT( \
      ...
      MO(_FUNC), KC_LGUI, KC_LALT,               KC_TRNS, NG_SHFTL, NG_SHFTR, KC_TRNS,       KC_TRNS, KC_TRNS, KC_TRNS,  KC_TRNS, KC_TRNS),
  ...
  [_FUNC] = LAYOUT( \
      ...
      MO(_FUNC),   KC_TRNS,  KC_TRNS,         KC_SPC,  KC_EISU,  KC_KANA2, KC_SPC,         KC_TRNS, KC_TRNS, KC_HOME,KC_PGDN, KC_END)
};
...
const rgblight_segment_t PROGMEM rgb_layer_1st[] = RGBLIGHT_LAYER_SEGMENTS(
    {0, 12, HSV_BLUE}, // 開始LEDインデックス, 連続する個数, 光らせる色(ここではプリセットを使用)
);
const rgblight_segment_t PROGMEM rgb_layer_2nd[] = RGBLIGHT_LAYER_SEGMENTS(
    {0, 12, HSV_GREEN}, // 開始LEDインデックス, 連続する個数, 光らせる色(ここではプリセットを使用)
);
